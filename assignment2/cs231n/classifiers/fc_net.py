...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    Ws={'W'+str(i+1) : np.random.randn(dims[i],dims[i+1])*weight_scale for i in xrange(self.num_layers)}
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers-1):
        if self.use_batchnorm:
            flow_in,cache=affine_bn_relu_forward(flow_in,
         					 self.params['W'+str(i+1)],
         					 self.params['b'+str(i+1)],
        					 self.params['gamma'+str(i+1)],
						 self.params['beta'+str(i+1)],self.bn_params[i])
        else:
            flow_in,cache=affine_relu_forward(flow_in,
         					 self.params['W'+str(i+1)],
         					 self.params['b'+str(i+1)])
        self.fwd_data['cache'+str(i+1)]=cache
        if self.use_dropout:
            flow_in,cache=dropout_forward(flow_in,self.dropout_param)
            self.fwd_data['dropcache'+str(i+1)]=cache
    flow_in,cache=affine_forward(flow_in,
				 self.params['W'+str(self.num_layers)],
				 self.params['b'+str(self.num_layers)])
    self.fwd_data['cache'+str(self.num_layers)]=cache
    scores=flow_in
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx,dW,db=affine_backward(dx,self.fwd_data['cache'+str(self.num_layers)])
    grads['W'+str(self.num_layers)]=dW
    grads['b'+str(self.num_layers)]=db
    for i in xrange(self.num_layers-2,-1,-1):
        if self.use_dropout:
            dx=dropout_backward(dx,self.fwd_data['dropcache'+str(i+1)])
        if self.use_batchnorm:
            dx,dW,db,dgamma,dbeta=affine_bn_relu_backward(dx,self.fwd_data['cache'+str(i+1)])
            grads['gamma'+str(i+1)]=dgamma
            grads['beta'+str(i+1)]=dbeta
        else:
            dx,dW,db=affine_relu_backward(dx,self.fwd_data['cache'+str(i+1)])
